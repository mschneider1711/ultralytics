...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x[:, :H, :W, :].contiguous()

        # calculate attention map dynamically in forward pass because of padding
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, hw_shape):
        ...
        hw_before_downsample = hw_shape

        if self.downsample is not None:
            x = self.downsample(x)
            new_hw_shape = (hw_shape[0] // 2, hw_shape[1] // 2)
            return x, new_hw_shape, x_before_downsample, hw_before_downsample
        else:
            return x, hw_shape, x_before_downsample, hw_before_downsample
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=640, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, pretrained_window_sizes=[8, 8, 8, 4], **kwargs):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Reshape output feature map
            B, L, C = out.shape
            H, W = out_hw_shape
            feat_map = out.transpose(1, 2).reshape(B, C, H, W)
