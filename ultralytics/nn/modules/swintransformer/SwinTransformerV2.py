...
class WindowAttention(nn.Module):
    ...
    Swin Transformer V2 Version mit verbesserter relativer Positionskodierung.
    """
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
        ...
        # V2: Skalierbare Logit-Skalierung
        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((num_heads, 1, 1))), requires_grad=True)

        # V2: MLP für kontinuierliche relative Position Bias
        ...
        # Normalisierung basierend auf pretrained window size
...
class SwinTransformerBlock(nn.Module):
    ...
    Angepasst für flexible Auflösungen ohne feste input_resolution.
    """
    ...
    def forward(self, x, mask_matrix):
        """ Forward function für Object Detection.
        Args:
            x: Input feature, tensor size (B, H*W, C).
            mask_matrix: Attention mask für cyclic shift.
        """
        B, L, C = x.shape
        H, W = self.H, self.W
...
class PatchMerging(nn.Module):
    """ Patch Merging Layer für Object Detection """
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False)
        self.norm = norm_layer(2 * dim)  # V2: Post-Norm
    ...
    def forward(self, x, H, W):
        ...
        x = self.reduction(x)
        x = self.norm(x)  # V2: Post-Norm

        return x


class BasicLayer(nn.Module):
    """ Basic Swin Transformer V2 Layer für Object Detection """
    ...
    def forward(self, x, H, W):
        # Calculate attention mask für SW-MSA
...
class PatchEmbed(nn.Module):
    """ Image to Patch Embedding für Object Detection """
...
class SwinTransformerV2(nn.Module):
    """ Swin Transformer V2 Backbone für Object Detection
    
    Hauptverbesserungen in V2:
    - Cosine attention statt dot-product
    - Kontinuierliche relative position bias via MLP
    - Skalierbare logit-Skalierung
    - Post-normalization in einigen Komponenten
    """
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96,
        ...
        # Add norm layer für jede Output-Stufe
