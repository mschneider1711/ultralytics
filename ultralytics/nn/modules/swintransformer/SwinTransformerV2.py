...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        clamp_max = torch.log(torch.tensor(1.0 / 0.01, device=self.logit_scale.device, dtype=self.logit_scale.dtype))
        logit_scale = torch.clamp(self.logit_scale, max=clamp_max).exp()
