...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            print(f"[INFO] Lade pretrained weights von: {pretrained}")
            ...
            for k, v in ckpt_state.items():
                # Falls Prefix (z. B. 'backbone.') nötig ist, hier anpassen
                if k in model_state and v.shape == model_state[k].shape:
                    loaded_state[k] = v

            # Logging Unterschiede
            ...
            matched_keys = list(loaded_state.keys())

            print(f"✅ Matching keys: {len(matched_keys)}")
            print(f"⚠️ Fehlende keys im Modell: {len(missing_keys)}")
            print(f"❌ Unused keys in Checkpoint: {len(unexpected_keys)}")

            if missing_keys:
                print("\n⚠️ Beispiel fehlende keys:")
                for k in missing_keys[:5]:
                    print(f"  - {k}")
            if unexpected_keys:
                print("\n❌ Beispiel unused keys:")
            ...
            print("[✅] Weights erfolgreich geladen.")
